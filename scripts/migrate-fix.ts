/**
 * Database Migration Script - Fixes existing schema
 * Run with: npx tsx scripts/migrate-fix.ts
 */
...
async function runMigration() {
    ...
    try {
        ...
        const idType = leadsInfo[0]?.data_type;
        console.log(`  Current leads.id type: ${idType || 'table does not exist'}`);

        // If leads table exists with integer ID, we need to migrate
        if (idType === 'integer') {
            console.log('\n⚠️  Found integer IDs - migrating to UUID schema...\n');

            // Drop existing tables in correct order (due to foreign keys)
            console.log('Dropping existing tables...');
            await sql`DROP TABLE IF EXISTS messages CASCADE`;
            await sql`DROP TABLE IF EXISTS tasks CASCADE`;
            await sql`DROP TABLE IF EXISTS rag_stores CASCADE`;
            await sql`DROP TABLE IF EXISTS leads CASCADE`;
            console.log('  ✓ Old tables dropped');
        }

        // Create fresh tables with UUID
        console.log('\nCreating tables with UUID schema...');
