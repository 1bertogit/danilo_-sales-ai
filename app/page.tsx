...
export default function Dashboard() {
    ...
    const handleUploadDocuments = async (files: File[]) => {
        if (!activeRagStoreName || files.length === 0) return;

        setIsUploading(true);
        try {
            await geminiService.uploadDocuments(activeRagStoreName, files);
            // Show success - could add toast notification here
            console.log(`Successfully uploaded ${files.length} documents to RAG store`);
        } catch (error) {
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="p-2 rounded-xl text-surface-400 hover:text-jade-600 hover:bg-jade-50 transition-all"
